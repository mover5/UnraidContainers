...
### flight-tracker
Mobile-first flight log with a web dashboard (port 8080). A single container running an Express API that serves a built React/Vite SPA from `./public`. Tap to stamp in-flight event times (gate push / takeoff / land / gate arrive) — works offline and syncs when back online — log timestamped notes, and visualize derived metrics (taxi-out, taxi-in, air time, block time, departure/arrival delays). Persists to an **external** Postgres via `DATABASE_URL` (no bundled DB; the included `docker-compose.yml` spins up a throwaway Postgres for local testing only). Seeds an initial set of flights on first run if the DB is empty (`SEED_ON_EMPTY`). Multi-stage Dockerfile: stage 1 builds the frontend with `VITE_API_URL=/api` baked in, stage 2 installs server deps only. This directory is the canonical source — the standalone `Flight-Tracker` repo was retired and folded in here, so do all development directly under `containers/flight-tracker/`. The `passengers` column is `text[]` (a list of people per flight); `init()` runs an idempotent migration that splits older single-string values on startup.

### tailscale-ikev2-gateway
IKEv2 VPN gateway into the Tailscale tailnet for devices that can't run the Tailscale client (e.g. locked-down Windows machines using only the built-in VPN client). Runs strongSwan (IKEv2 + EAP-MSCHAPv2 username/password auth) and tailscaled in one Alpine container; VPN-client traffic destined for `100.64.0.0/10` is NATed out `tailscale0`, so clients reach tailnet nodes without subnet routing. Server cert comes from Let's Encrypt via Cloudflare DNS-01 (`CF_DNS_API_TOKEN`, acme.sh) so Windows needs no cert import; falls back to a self-signed CA persisted in `/data/pki`. Optional Cloudflare DDNS loop keeps `VPN_DOMAIN`'s A record on the current WAN IP. Needs `NET_ADMIN`, `/dev/net/tun`, `--sysctl net.ipv4.ip_forward=1`, and UDP 500/4500 forwarded on the router. Split tunneling is configured client-side (PowerShell `Add-VpnConnectionRoute 100.64.0.0/10`); see the container README for the full Windows setup.

### wine-tracker
A tiny wine cellar tracker with a web dashboard (port 8080). A single container running an Express API that also serves a self-contained static SPA (plain HTML/JS, no build step) from `./public`. Log bottles by name + year, record the date won, and mark bottles drunk (with a date) to move them from the **Cellar** view (in stock) into the **History** view. Includes search and a soft duplicate warning. Persists to a **local SQLite** file at `DB_PATH` (default `/data/wine.db`) via `better-sqlite3` — no external database; map `/data` to persistent storage to survive reboots/updates. Multi-stage Dockerfile on `node:22-bookworm-slim`: stage 1 installs deps (with python3/make/g++ so the `better-sqlite3` native addon compiles if no prebuild is available), stage 2 copies `node_modules` + app into a lean runtime. Schema (one `wines` table) is created on startup; a null `drank_date` means the bottle is still in the cellar.
